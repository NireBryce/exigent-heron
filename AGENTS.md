...
**The critical structural rule:** `domain/` has zero Android imports. `RuleEngine`, `SecretDetector`, and `Deduplicator` are Android-free Kotlin, unit-testable on the JVM with no Robolectric, no instrumentation, no emulator. This is what keeps the logic most worth testing the cheapest thing here to test, and it stays a hard requirement — `.github/workflows/check.yml` enforces it with a grep rather than leaving it to review.

**What that rule is not:** it is not a ban on testing the Android side, and "glue contains no logic worth testing" is the goal the boundary exists to serve, not a description of what's already true. `AppContainer` decides real things when it builds the lambdas it hands the gates; the worst pipeline bug so far — the output-route gate checked once at enqueue time instead of per utterance — lived in a seam that every JVM test passed straight through, and was found on an emulator. Instrumented tests under `app/src/androidTest/` are the right tool for that class of thing: DataStore round-trips, `AppContainer`'s wiring, `AndroidTtsEngine`'s real callback behaviour, `<queries>`-dependent package lookups. The order of preference is move the decision into `domain/` where it can move, instrument it where it can't, and leave it to [wiki/testing.md](wiki/testing.md)'s manual script only when neither works. Robolectric stays out of §2's list — the dev shell already ships a real emulator, and a simulated framework is the wrong place to learn framework facts.
...
- Unit tests pass and cover the Android-free domain layer
