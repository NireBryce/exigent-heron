...
Added **2026-09-08**, and separate from the phase table above because
they cut across phases rather than belonging to one. Both were run on the
`nix develop` emulator (`dev`, API 37, `google_apis` x86_64) that day —
still not a physical device, the same caveat every emulator row above
carries and `SECURITY.md` §4 spells out.

| Check | Command | Verified |
|---|---|---|
| Instrumented tests | `gradle connectedDebugAndroidTest` | **2026-09-08**: 19 tests, 0 failures, 0 skipped — result XML read directly, confirming the TTS cases actually ran rather than being skipped by their `assumeTrue` guard. |
| Listener acceptance | `./scripts/listener-acceptance.sh` | **2026-09-08**: PASS, three consecutive runs. Automates Phase 2's "spoken once" and "three duplicates still once" against real posted notifications; the duck-and-recover and headset-routing criteria still need ears and stay manual in [testing.md](testing.md). |
